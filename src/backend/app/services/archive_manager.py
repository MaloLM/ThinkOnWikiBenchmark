...
class ArchiveManager:
    ...
    def __init__(self, base_path: str = "archives"):
        ...
        logger.info(f"Archive manager initialized with base path: {self.base_path}")
    ...
    def save_step(self, run_id: str, step_index: int, step_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to save step {step_index} for run {run_id}: {e}")
    ...
    def save_model_step(
        ...
        try:
            ...
            logger.error(f"Failed to save step {step_index} for model {model_name}: {e}")
    ...
    def save_model_metrics(self, run_id: str, model_name: str, metrics: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"Saved metrics for model {model_name} in run {run_id}")
        except Exception as e:
                ...
                if run_dir.is_dir():
                    ...
                    if config_path.exists():
                        try:
                            ...
                            logger.warning(f"Failed to load archive {run_dir.name}: {e}")
        except Exception as e:
    ...
    def get_archive_details(self, run_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get archive details for run {run_id}: {e}")
            return None
